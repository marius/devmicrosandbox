...
After completing the memory writes the user requested, ask:
...
3. Did I add or update concepts? $\rightarrow$ Ensure `knowledge/log.md` and the parent `index.md` reflect the changes.
4. Did I write raw scratchpads or chat noise? $\rightarrow$ **Clean up and delete them.**
5. Did I run `okf validate knowledge --strict`? $\rightarrow$ Ensure 0 errors.
