...
1. **Opt-In Writes**: Only write to `knowledge/` when the user explicitly requests memory work (e.g. "document this in OKF", "remember this"). Never record knowledge automatically after tasks, features, or decisions.
2. **Persistent knowledge lives in the OKF corpus**: Conversations are temporary; the `knowledge/` directory survives.
3. **Search Before Write**: Always query existing knowledge before authoring new concepts.
4. **No Blanket Scans**: Never use `list_dir`, `grep`, or dump `knowledge/` in bulk. Query via `okf search` and load concepts on demand via `okf show`.
...
8. **End-of-Task Review**: After completing the memory writes the user requested, run the checklist in [remember.md §6](./remember.md#6-end-of-task-knowledge-checklist).
9. **Always Validate**: Ensure `okf validate knowledge --strict --drift` passes with 0 errors and 0 warnings.

---

## 2. Tooling Commands Reference

Use the `okf` CLI for deterministic operations:
